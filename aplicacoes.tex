...
\begin{lema}{Lema}{} Seja $c_n = \frac{(2n - 2)}{2^{2n-1}n!(n-1)!} = \frac{1}{n}2^{1-2n}\binom{2n-2}{n-1}, / n \geq 1 $. Então para
$x \in (-1,1)$,
...
Voltando agora ao problema inicial, sejam $M$ o evento em que a mutação aparece na populaçãop e $M^c$ o complementar de $M$.
...
\begin{definicao}{Aproximação linear}{}
Dada uma função $f(x)$ contínua, diferenciável e com variável real x,
...
Usando as aproximações de $f_1$ e $f_2$ conseguimos para $p < \frac{1}{2}$ fixo a seguinte aproximação para nossa expressão
